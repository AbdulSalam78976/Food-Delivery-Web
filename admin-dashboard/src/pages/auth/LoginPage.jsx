...
import { formatRemainingTime } from '@/services/loginAttempts';
import { getRememberMePreference } from '@/services/sessionManager';
...
const LoginPage = () => {
  ...
  useEffect(() => {
    const { isRemembered, rememberedEmail } = getRememberMePreference();
    ...
    try {
      ...
      if (error) {
        if (error.code === 'account_locked') {
          setErrors({ 
            general: `${error.message} Time remaining: ${formatRemainingTime(error.remainingTime)}`
          });
